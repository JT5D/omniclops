...
int main(int argc, char* argv[]) {
	...
	opt->addUsage( "     --strut2                Angle of strut 2 in degrees");
	opt->addUsage( "     --strut3                Angle of strut 3 in degrees");
	opt->addUsage( "     --strutwidth            Width of struts in degrees");
	...
	opt->setOption(  "centrex" );
	opt->setOption(  "centrey" );
	opt->setOption(  "inneraspect" );
	opt->setOption(  "strut1" );
	opt->setOption(  "strut2" );
	opt->setOption(  "strut3" );
	opt->setOption(  "strutwidth" );
	opt->setOption(  "fast" );
	opt->setOption(  "diam" );
	opt->setOption(  "dist" );
	opt->setOption(  "focal" );
	...
	if( opt->getValue( "strut2" ) != NULL  ) {
		strut_angles[1] = atof(opt->getValue("strut2"));
		no_of_struts=2;
	}
	if( opt->getValue( "strut3" ) != NULL  ) {
		strut_angles[2] = atof(opt->getValue("strut3"));
		no_of_struts=3;
	}
	if( opt->getValue( "strutwidth" ) != NULL  ) {
		strut_width = atof(opt->getValue("strutwidth"));
	}
	...
	*/
	...
	while(1){
		...
		lcam->remove(l_, ww, hh, 3, centre_x, centre_y, outer_radius,
			inner_rad, inner_aspect, no_of_struts, strut_angles, strut_width);
		...
		if ((unwarp_features) && (!show_motion)) {
